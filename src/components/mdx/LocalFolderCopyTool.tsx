...
function parseRules(rules: string[]): ParsedRule[] {
  return rules
    ...
    .map((rule): ParsedRule => {
      ...
      if (m) {
        try {
          return { type: "regex", src: rule, re: new RegExp(m[1], m[2]) };
        } catch {
          /* fallthrough */
        }
      }
...
function canDecode(bytes: Uint8Array, enc: string): boolean {
  try {
    return looksLikeText(new TextDecoder(enc, { fatal: true }).decode(bytes));
  } catch {
    return false;
  }
}
function detectEncoding(bytes: Uint8Array): string | null {
  ...
  if (bom && canDecode(bytes, bom)) return bom;
  const u16 = detectUtf16(bytes);
  if (u16 && canDecode(bytes, u16)) return u16;
...
async function readFileText(handle: FileSystemFileHandle): Promise<ReadResult> {
  ...
  try {
    ...
    return { ok: false, reason: "無法讀取" };
  }
  if (file.size > CFG.maxFileSize)
    return { ok: false, reason: `超過 ${fmtBytes(CFG.maxFileSize)}` };
  let buf: ArrayBuffer;
  try {
...
const hasAncIn = (p: string, s: Set<string>): boolean => {
  for (const x of s) if (isDescOf(p, x)) return true;
  return false;
};
const hasDescIn = (p: string, s: Set<string>): boolean => {
  for (const x of s) if (x !== p && x.startsWith(p + "/")) return true;
  return false;
};
...
function setAdd(s: Set<string>, v: string): Set<string> {
  const n = cloneSet(s);
  n.add(v);
  return n;
}
function setDel(s: Set<string>, v: string): Set<string> {
  const n = cloneSet(s);
  n.delete(v);
  return n;
}
function setRmDesc(s: Set<string>, p: string): Set<string> {
  const n = cloneSet(s);
  for (const x of cloneSet(n)) if (isDescOf(x, p)) n.delete(x);
  return n;
}
function setRmAnc(s: Set<string>, p: string): Set<string> {
  const n = cloneSet(s);
  for (const x of cloneSet(n)) if (x !== p && p.startsWith(x + "/")) n.delete(x);
  return n;
}
...
const IconChevron: FC = () =>
  h("svg", { viewBox: "0 0 24 24", ...svgBase }, h("polyline", { points: "9 18 15 12 9 6" }));

const IconFolder: FC = () =>
  h(
    "svg",
    { className: "lf-icon lf-icon-folder", viewBox: "0 0 24 24", ...svgBase },
...
const IconFile: FC = () =>
  h(
    "svg",
    { className: "lf-icon", viewBox: "0 0 24 24", ...svgBase },
...
const IconX: FC = () =>
  h(
    "svg",
    { viewBox: "0 0 24 24", ...svgBase, style: { width: 14, height: 14 } },
...
const IconCheck: FC = () =>
  h(
    "svg",
    { viewBox: "0 0 24 24", ...svgBase, strokeWidth: 2.5 },
...
const IconFolderPlus: FC = () =>
  h(
    "svg",
    { viewBox: "0 0 24 24", ...svgBase, strokeWidth: 1.8 },
    h("path", { d: "M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" }),
...
const IconChevronDown: FC = () =>
  h("svg", { viewBox: "0 0 24 24", ...svgBase }, h("polyline", { points: "6 9 12 15 18 9" }));

const IconInfo: FC = () =>
  h(
    "svg",
    { viewBox: "0 0 24 24", ...svgBase, width: 17, height: 17 },
    h("circle", { cx: 12, cy: 12, r: 10 }),
...
function useToasts() {
  ...
  const show = useCallback((msg: string, type: ToastItem["type"] = "info") => {
    ...
    setTimeout(
      () => setToasts((p) => p.map((t) => (t.id === id ? { ...t, visible: true } : t))),
      30,
    );
...
const SettingsPanel: FC<SettingsProps> = ({ rules, toast, onSave, onClose }) => {
  ...
  const add = () => {
    const v = input.trim();
    if (!v) return;
    if (editing.some((x) => x.toLowerCase() === v.toLowerCase())) {
      toast(`「${v}」已在清單中`, "error");
      return;
    }
  ...
  return h(
    "div",
    { className: "lf-settings" },
    h(
      "p",
      { className: "lf-settings-hint" },
      "支援資料夾 / 檔案名稱，或正規表達式如 ",
      h("code", null, "/\\.log$/i"),
    ),
    h(
    ...
    h(
      "div",
      { className: "lf-settings-list" },
      editing.length === 0
        ? h("div", { className: "lf-settings-empty" }, "目前無略過項目")
        : editing.map((r, i) =>
            h(
              "div",
              { key: `${r}-${i}`, className: "lf-settings-item" },
              h("span", null, r),
    ...
    h(
      "div",
      { className: "lf-settings-foot" },
      h(
        "button",
        { className: "lf-link", onClick: () => setEditing([...DEFAULT_IGNORES]) },
        "還原預設",
      ),
      h(
        "button",
...
const LocalFolderCopyTool: FC = () => {
  const [rules, setRules] = useState<string[]>(() =>
    typeof window !== "undefined" ? (loadRules() ?? [...DEFAULT_IGNORES]) : [...DEFAULT_IGNORES],
  );
    ...
    (node: FileNode): SelectionState => {
      ...
      if (node.loaded && node.children && node.children.length > 0) {
        ...
        if (sc.length > 0) {
          ...
          if (cs.every((s) => s.checked && !s.indeterminate))
            return { checked: true, indeterminate: false };
          if (cs.some((s) => s.checked || s.indeterminate))
            return { checked: false, indeterminate: true };
        }
      }
      if (hasDescIn(p, selFiles) || hasDescIn(p, selFolders))
        return { checked: false, indeterminate: true };
  ...
  const toggleSelect = useCallback(
    (node: FileNode) => {
      ...
      if (st.indeterminate || !st.checked) {
        ...
        if (byAnc) {
          if (selFolders.has(p)) {
            setSelFolders(setDel(selFolders, p));
            setSelFiles(setRmDesc(selFiles, p));
            setExclPaths(setRmDesc(exclPaths, p));
          } else {
            setSelFiles(setRmDesc(selFiles, p));
            setSelFolders(setRmDesc(selFolders, p));
            setExclPaths(setAdd(setRmDesc(exclPaths, p), p));
          }
        } else if (node.kind === "directory") {
          setSelFolders(setRmDesc(setDel(selFolders, p), p));
          setSelFiles(setRmDesc(selFiles, p));
          setExclPaths(setRmDesc(exclPaths, p));
        } else {
          setSelFiles(setDel(selFiles, p));
          setExclPaths(setRmDesc(exclPaths, p));
        }
      }
    },
  ...
  const pickDirectory = async () => {
    if (typeof window.showDirectoryPicker !== "function") {
      toast("請使用 Chrome 或 Edge 瀏覽器", "error");
      return;
    }
    try {
        ...
        if (item.name.toLowerCase().includes(q)) {
          ...
          if (!batchTimer) {
            batchTimer = setTimeout(() => {
              if (!gen.cancelled) setSearchResults([...results]);
              batchTimer = null;
            }, 150);
          }
        }
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const recordSkip = (path: string, reason: string) => {
        skipped++;
        if (skippedPreview.length < 5) skippedPreview.push({ path, reason });
      };
      const append = (text: string, path: string): boolean => {
        if (limitHit) return false;
        const out = `----- BEGIN FILE: ${path} -----\n\n${text}\n\n----- END FILE: ${path} -----`;
        if (totalChars + out.length > CFG.maxOutput) {
          chunks.push("\n----- OUTPUT TRUNCATED -----");
          limitHit = true;
          return false;
        }
      ...
      async function processFile(handle: FileSystemFileHandle, path: string) {
        ...
        if (r.ok && r.text) {
          if (!append(r.text, path)) recordSkip(path, "超過上限");
      ...
      for (const fp of selFolders) {
        if (limitHit) break;
        let covered = false;
        for (const f2 of selFolders) {
          if (f2 !== fp && fp.startsWith(f2 + "/")) {
            covered = true;
            break;
          }
        }
      ...
      if (!output.trim()) {
        toast(skipped > 0 ? `無可複製內容，已略過 ${skipped} 個檔案` : "無可複製的文字檔", "error");
        return;
      }
      await navigator.clipboard.writeText(output);
      toast(
        `成功複製 ${copied} 個檔案（${output.length.toLocaleString()} 字元${limitHit ? "，已截斷" : ""}）`,
        "success",
      );
      if (skipped > 0) {
        toast(
          `已略過 ${skipped} 個：${skippedPreview.map((s) => `${s.path}(${s.reason})`).join("、")}`,
          "warn",
        );
      }
    } catch {
      toast("複製失敗，請確認剪貼簿權限", "error");
    } finally {
      setIsCopying(false);
    }
  };
  ...
  const renderNodes = (nodes: FileNode[]): ReactNode[] =>
    nodes.map((node) => {
      ...
      return h(
        "div",
        { key: node.path, className: "lf-node" },
        h(
          "div",
          {
            className: cx(
              "lf-row",
              st.checked && "lf-row--sel",
              st.indeterminate && "lf-row--part",
              node.ignored && "lf-row--ign",
            ),
            onClick: () => {
              if (!node.ignored) toggleSelect(node);
            },
          },
          isDir && !node.ignored
            ? h(
                "button",
                {
                  className: cx("lf-exp", exp && "lf-exp--open"),
                  onClick: (e: RMouseEvent) => {
                    e.stopPropagation();
          ...
          !node.ignored
            ? h(
                "div",
                { className: "lf-row-acts" },
                h(
                  "button",
                  {
                    className: "lf-excl-btn",
                    title: "加入略過清單",
                    onClick: (e: RMouseEvent) => {
                      e.stopPropagation();
                      excludeName(node.name);
                    },
                  },
                  h(IconExclude),
                ),
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              h(
                "div",
                null,
                h("h1", { className: "lf-title" }, "Folder Copy"),
                h("p", { className: "lf-desc" }, "選擇資料夾，勾選檔案，一鍵複製"),
              ),
            ),
            h(
              "button",
              { className: cx("lf-pick", loaded && "lf-pick--loaded"), onClick: pickDirectory },
              h(
                "div",
                { className: "lf-pick-inner" },
                h(IconFolderPlus),
                h("span", null, loaded ? "重新選擇" : "選擇資料夾"),
              ),
            ...
            loaded
              ? h(
                  ...
                  h("input", {
                    ...
                    placeholder: "搜尋所有檔案…",
                    value: searchQ,
                    onChange: (e: ChangeEvent<HTMLInputElement>) => setSearchQ(e.target.value),
                  }),
                  isSearching ? h("span", { className: "lf-search-spin" }, "⟳") : null,
                )
              : null,
            ...
            h(
              "button",
              {
                className: cx("lf-acc-trigger", showSettings && "lf-acc-trigger--open"),
                onClick: () => setShowSettings(!showSettings),
              },
              h("span", { className: "lf-acc-left" }, h(IconSettings), " 略過清單"),
              h(IconChevronDown),
            ),
            showSettings
              ? h(SettingsPanel, {
          ...
          loaded
            ? h(
                "div",
                { className: "lf-side-foot" },
                h(
                  "div",
                  { className: "lf-foot-meta" },
                  h("span", { className: "lf-count" }, countText),
                  h(
                    "button",
                    {
                      className: "lf-link",
                      disabled: totalSelected === 0 || isCopying,
                      onClick: clearAll,
                    },
                    "清除",
                  ),
                ...
                h(
                  ...
                  isCopying
                    ? h("span", null, "⏳ 複製中…")
                    : [h(IconCopy, { key: "ic" }), h("span", { key: "tx" }, "複製勾選內容")],
                ),
              )
            : null,
        ),
        h(
          ...
          !loaded
            ? h(
                ...
                h(
                  "div",
                  { className: "lf-empty-gfx" },
                  h("div", { className: "lf-pulse lf-pulse--lg" }),
                  h("div", { className: "lf-pulse lf-pulse--sm" }),
                  h(IconFolderPlus),
                ),
                h("h3", null, "選擇你的專案資料夾"),
                h("p", null, "點擊左側「選擇資料夾」開始"),
              )
            : null,
          loaded && !isSearchMode
            ? h(
                "div",
                { className: "lf-tree-wrap" },
                h("div", { className: "lf-tree" }, renderNodes(tree)),
              )
            : null,
          loaded && isSearchMode
            ? h(
                "div",
                { className: "lf-sr-wrap" },
                h(
                  "div",
                  { className: "lf-sr-header" },
                  h(
                    "span",
                    { className: "lf-sr-status" },
                    isSearching ? "搜尋中…" : `找到 ${searchResults.length} 個結果`,
                  ),
                ),
                h(
                  "div",
                  { className: "lf-sr-list" },
                  searchResults.length === 0 && !isSearching
                    ? h("div", { className: "lf-sr-none" }, "沒有符合的結果")
                    : null,
