...
:::warning
`bun run build` 是兩個步驟：`build:site`（`astro build`）加上 `build:index`（`pagefind`）。
站內搜尋的索引是 Pagefind 在 build 時掃描 `dist/` 產生的，
所以在 CI 上**一定要呼叫 `bun run build`**，只跑 `astro build` 會讓搜尋功能失效。
:::
...
| Build command                       | `bun run build`   | 不需要自己加 `bun install`，Cloudflare 會先裝好依賴，見下方                        |
...
flowchart LR
    ...
    B --> C["GitHub Actions<br/>bun run build"]
